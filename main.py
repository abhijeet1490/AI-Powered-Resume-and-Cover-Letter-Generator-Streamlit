...
if not GOOGLE_API_KEY:  
    raise ValueError("No Google API key found. Please set it in your environment variables.")
...
def generate_cover_letter(prompt, temperature=0.99):
    try:
        # Update the model temperature for this specific call
...
def display_and_download(text, file_label):
    ...
    # Download as Text File only
    st.download_button(
        f'📄 Download {file_label} as TXT', 
        text, 
        file_name=f"{file_label}.txt",
